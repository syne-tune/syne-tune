...
def plot_hyp_per(ax, x_par_raw, y_par_raw, ii_ch, res, data_sizes, n_mark=10, 
                 plot_x_label=True, plot_y_label=True, plot_title=True,
                 plot_colour_bar=True, vmin=None, vmax=None, marker_size=100,
                 fontsize=14):
    ...
    for pt_idx in range(N_hyp):
        tup = (res['parameters_mat'][x_par][pt_idx],
    ...
    for tup in vals:
        cb = ax.scatter(tup[0],
                        ...
                        alpha=0.5,
                        c=np.mean(vals[tup]),
                        norm=plc.LogNorm(vmin=vmin, vmax=vmax), s=15
                       )
    ...
    sorted_idx = [x for _, x in sorted(zip(np.array(res['test_error_mat'])[ii_ch,:,0], idx))]
    ax.scatter(np.array(res['parameters_mat'][x_par])[sorted_idx[:n_mark]],
               np.array(res['parameters_mat'][y_par])[sorted_idx[:n_mark]], marker='X', c='k',
               s=marker_size, edgecolors='white', linewidths=1.75)
    return cb
...
for pp in range(N_plot):
    ...
    cb = plot_hyp_per(ax[pp], par_x, par_y, ii_ch, agg_experiments, 
                 agg_experiments['data_sizes'], plot_y_label=(pp==0),
                 plot_colour_bar=False, vmin=vmin, vmax=vmax, fontsize=14)
...
par_combos = [('learning_rate', 'max_depth'), ('learning_rate', 'n_estimators'), 
              ('learning_rate', 'min_child_weight'),
              ('max_depth', 'n_estimators'), ('max_depth', 'min_child_weight'),
...
for pp in range(len(par_combos)):
    ...
    plot_hyp_per(ax[pp, 0], par_x, par_y, 0, 
                 agg_experiments, agg_experiments['data_sizes'], plot_title=(pp==0), marker_size=250, fontsize=28)
    plot_hyp_per(ax[pp, 1], par_x, par_y, 27,
                 agg_experiments, agg_experiments['data_sizes'], plot_y_label=False, plot_title=(pp==0), marker_size=250,
                fontsize=28)
