...
experiments = ['YAHPO_auc_svm_1220', 'YAHPO_auc_aknn_4538', 'YAHPO_auc_ranger_4154', 'YAHPO_auc_glmnet_375',
              'YAHPO_auc_svm_458', 'YAHPO_auc_aknn_41138', 'YAHPO_auc_ranger_40978', 'YAHPO_auc_glmnet_40981']
...
for optimisers_unsorted in [
    ...
    for idx in range(len(experiments)):
        ...
        preprocessed = pickle.load(open('plotting/preprocessed_results_%s.p' %experiment, 'rb'))
        ...
        for task_val in preprocessed['task_values'][1:]:
            ...
            for optimiser in optimisers:
                ...
                ll = axes[ax2,ax1].errorbar(task_num,
                             mean_val[it-1], yerr=2*std_val[it-1], fmt=marker[optimiser],
                            c=colours[optimiser], label=labels[optimiser])

                if task_val == preprocessed['task_values'][1] and experiment == experiments[0]:
    ...
    axes[1,1].legend(handles=anchor_list, labels=label_list,loc='upper center', frameon=False,
        bbox_to_anchor=(1.1, -0.25),fancybox=False, shadow=False, ncol=4)
