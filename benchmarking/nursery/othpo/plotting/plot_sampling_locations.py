...
def add_samples_kde(ax, task_id, optimiser, res_to_plot, num_its, label, anchors, hyp_x, hyp_y, alpha):
    ...
    for seed in range(50):
        for it in range(num_its):
            ...
            xx_list.append(xx)
            yy_list.append(yy)
    aa = ax.scatter(xx_list, yy_list, c=colours[optimiser], label=label,
                    s=50, zorder=10, alpha=alpha)

    df = pd.DataFrame({hyp_x: xx_list, hyp_y: yy_list})
    sns.kdeplot(df, x=hyp_x, y=hyp_y, color=colours[optimiser],
                ax=ax, alpha=0.5, fill=True, zorder=0)
...
task_values, get_backend = get_configs(backend, yahpo_dataset=dataset, yahpo_scenario=scenario)
...
for idx in range(N_plot):
    fig_sub, ax_sub = plot_hp_perf(
        scenario='%s' %scenario, instance=dataset, metric=experiments_meta_data['metric'],
        train_fracs=train_fracs, x_par=hyp_x, y_par=hyp_y, log_x=True, log_y=True,
        marker_colour='black', task_values=task_values, save_fig=False, 
        alpha_back=0.5, alpha_crosses=1, fig_axes=(fig, axes[idx,:]), only_crosses=True,
        edgewidth=0, cross_size=50)
...
for ax_idx in range(N_plot):
    ...
    for task_idx, task_id in enumerate([int(20*tf) for tf in train_fracs]):
        ...
        anchors = add_samples_kde(axes[ax_idx,task_idx], task_id, optimiser,
                        res_to_plot, num_its, label, anchors, hyp_x, hyp_y, alpha)
        label = None
...
for ax_idx in range(N_plot):
    for task_idx in range(len(train_fracs)):
        ...
        if ax_idx == 0:
            if task_idx in [0, 2]:
                axes[0,task_idx].set_title(axes[0,task_idx].get_title()+'\n first evaluation')
...
axes[ax_idx, 2].legend(handles=ordered_handles,
                       labels=ordered_labels,
                       loc='upper center',
             bbox_to_anchor=(0.0, -0.4),fancybox=False, shadow=False, ncol=4, frameon=False)
