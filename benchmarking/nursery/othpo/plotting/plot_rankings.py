...
def get_rankings(preprocessed, task_id, ii):
    ...
    tmp = {method: preprocessed['mean_normalised_score'][method][task_id][ii]
        for method in preprocessed['methods']}
    return {method: np.sum(np.array(list(tmp.values())) <= tmp[method]) for method in tmp}
...
for file, experiment in [
    ...
    for ii in range(25):
        ...
        for method in ranking_dicts:
            ...
            std_mean_ranking_dicts[method].append(np.std(ranking_dicts[method], ddof=1)/np.sqrt(len(preprocessed['task_values'])))
    ...
    for method in methods_to_use:
        if ax_idx == 0:
            ...
            label = None
        aa = ax.plot(range(1, 26), mean_ranking_dicts[method], 'd-', color=colours[method], linewidth=5,
                 markersize=10, label=label)
        if ax_idx == 0:
            anchor_list.append(aa[0])
        ax.plot(range(1, 26), np.array(mean_ranking_dicts[method])+2*np.array(std_mean_ranking_dicts[method]), 
                 ':', color=colours[method], alpha=0.3)
        ax.plot(range(1, 26), np.array(mean_ranking_dicts[method])-2*np.array(std_mean_ranking_dicts[method]), 
                 ':', color=colours[method], alpha=0.3)
        ax.fill_between(range(1, 26), 
                         np.array(mean_ranking_dicts[method])-2*np.array(std_mean_ranking_dicts[method]),
                         np.array(mean_ranking_dicts[method])+2*np.array(std_mean_ranking_dicts[method]),
...
axes[1].legend(handles=ordered_handles,
