...
from plotting_helper import colours, labels, legend_order, task_pos_in_order, sort_legend_labels, hatches, get_task_values_to_plot

from experiment_master_file import experiments_meta_dict, get_exp_title, get_task_values



def plot_bar(ax, method, label, dist, preprocessed_results, ylim_upper, task_val, it, width):
    normalisation_score = preprocessed_results['mean_normalised_score'][method][task_val][it-1]
    std_normalisation_score = preprocessed_results['std_mean_normalised_score'][method][task_val][it-1]

    ll, = ax.bar(dist, min(normalisation_score, ylim_upper), yerr=2*std_normalisation_score,
                 error_kw={'elinewidth': 2}, ecolor='black',
                 color=colours[method], width=width, alpha=0.8, label=label,
                 hatch=hatches[method], edgecolor='white', linewidth=0)

    if normalisation_score > ylim_upper:
        # Add arrow to show that value is off plot
        ax.errorbar(x=[dist], y=[ylim_upper], yerr=[ylim_upper/50],
            ecolor='black', capsize=3, capthick=0.1, lolims=3)

    elif normalisation_score+2*std_normalisation_score > ylim_upper:
        ax.errorbar(x=[dist], y=[ylim_upper], yerr=[ylim_upper/50],
            ecolor='dimgrey', capsize=3, capthick=0.1, lolims=3,
                        elinewidth=0)
...
def prepare_ax(axes, it, exp_idx, experiments_list, experiments_meta_dict):
    ...
    task_labels = [''] + [str(task_pos_in_order(val, task_values)) for val in task_values_to_plot] + ['']
    ax.set_xticklabels(task_labels)
    
    ax.set_ylabel('%s, %s it \n Normalised score' %(
        get_exp_title(experiments_meta_data), it))
...
def plot_bars(it, ylim_upper):
    ...
    for exp_idx, experiment in enumerate(experiments_list):
        ...
        preprocessed_results = pickle.load(open('plotting/preprocessed_results_%s.p' %experiment, 'rb'))
        num_methods = len(preprocessed_results['methods'])
        sorted_methods = sorted(preprocessed_results['methods'], key= lambda xx: legend_order[xx])
        ...
        for task_val_idx, task_val in enumerate(task_values_to_plot):
            ...
            for method in sorted_methods:
                ...
                ll = plot_bar(ax_bars, method, label, dist, preprocessed_results,
                              ylim_upper[experiment], task_val, it, width)
    ...
    ax_bars_mult[-1].legend(handles=ordered_handles,
                           labels=ordered_labels,
                           loc='upper center',
                 bbox_to_anchor=(0.5, -0.4),fancybox=False, shadow=False, ncol=4, frameon=False)
...
ylim_upper = {1:
              {'SimOpt': 500,
              'XGBoost': 400,
              'YAHPO_auc_svm_1220': 400},
              10:
              {'SimOpt': 400,
              'XGBoost': 100,
              'YAHPO_auc_svm_1220': 150},
              25:
              {'SimOpt': 100,
              'XGBoost': 30,
              'YAHPO_auc_svm_1220': 30}
             }
for it in [1, 10, 25]:
