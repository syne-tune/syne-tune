...
from plotting_helper import colours, linestyles, sort_legend_labels, get_task_values_to_plot, task_pos_in_order
...
def plot_mode(ax, preprocessed_results, method, task_val, colour):
    ...
    return ax.plot(range(1, preprocessed_results['points_per_task']+1), mean_method, c=colour, label=method,
...
for experiment in ['SimOpt', 'XGBoost', 'YAHPO_auc_svm_1220']:
    ...
    for aa in range(len(task_vals_to_plot)):
        ...
        for method in preprocessed_results['methods']:
            ll = plot_mode(ax, preprocessed_results, method,
    ...
    axes[2].legend(handles=ordered_handles,
