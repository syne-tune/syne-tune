...
def plot_hp_perf(scenario: str, instance: str, metric: str, train_fracs: List[float],
                 x_par, y_par, log_x=True, log_y=True, marker_colour='black', task_values=None,
                save_fig=True, alpha_back=1, alpha_crosses=1, fig_axes=None, only_crosses=False,
                edgewidth=1.75, cross_size=100):

    hp_names = [hp for hp in hp_names_raw[scenario]
    ...
    for j, frac in enumerate(train_fracs):
        ...
        if not only_crosses:
            sc = axes[j].scatter(perf_df[x_par], perf_df[y_par], c=perf_df[metric], s=5,
                                 vmin=np.min(df[metric]), vmax=np.max(df[metric]),
                                 alpha=alpha_back, zorder=0)

        inds = perf_df[metric].reset_index().nlargest(10, columns=metric).index
        for k, ind in enumerate(inds):
            axes[j].scatter(perf_df[x_par].loc[ind], perf_df[y_par].loc[ind], marker='X',
                            color=marker_colour, s=cross_size, edgecolors='white', linewidths=edgewidth,
                            alpha=alpha_crosses, zorder=20)
...
if __name__ == "__main__":
    ...
    for scenario in ['rbv2_svm', 'rbv2_aknn', 'rbv2_ranger', 'rbv2_glmnet']:
        for instance in instances[scenario]:
            ...
            log_axis = {'rbv2_svm': (True, True),
                        'rbv2_aknn': (False, False),
                        'rbv2_ranger': (False, False),
                        'rbv2_glmnet': (True, True)}[scenario]


            x_par, y_par = pars_to_plot[scenario]
            task_values = get_configs('YAHPO', yahpo_dataset=instance, yahpo_scenario=scenario)[0]

            _ = plot_hp_perf(scenario, instance, metric='auc', train_fracs=train_fracs,
                             x_par=x_par, y_par=y_par, log_x=log_axis[0], log_y=log_axis[1],
                             marker_colour='black', task_values=task_values)