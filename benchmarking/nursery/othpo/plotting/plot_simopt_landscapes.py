...
try:
    ...
    for time_idx in range(num_probs):
        ...
        context_file = str(file_dir)+'/opt-price-random-walk-utility-context-2022-11-08.p'
    ...
    for ii in range(0, N):
        for jj in range(0, N):
            xx = np.array([ii, jj, 8])
            for key in problems:
                results[key].append(evaluate_problem_price(problems[key], xx, rand_gen, reps=100))
...
for key in [0, 1, 4, 6, 8]:
    ...
    cb = plot_problem(ax=ax[ii], ii=ii, results=results[key], task_num=task_num,
                      N=N, num_cols=5, vmin=vmin, vmax=vmax)
    if ii != 0:
