...
for experiment in ['SimOpt', 'XGBoost', 'YAHPO_auc_svm_1220']:
    ...
    for method in ['WarmBO', 'Quantiles']:
        to_make_df['%s mean' %method] = [preprocessed_results['mean_performance'][method][task_val][it] for task_val in task_values]
        to_make_df['%s mean std' %method] = [preprocessed_results['std_mean_performance'][method][task_val][it] for task_val in task_values]
    ...
    df['perc. reduction std'] = 100*(1-df['WarmBO mean std']/df['Quantiles mean std'])
    ...
    print('Perc. reduction std: %s (%s -- %s)' %(mean_print_std, low_print_std, high_print_std))