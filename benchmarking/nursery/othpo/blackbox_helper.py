...
from syne_tune.blackbox_repository import add_surrogate, BlackboxRepositoryBackend, UserBlackboxBackend
...
from syne_tune.optimizer.schedulers.searchers.botorch.botorch_transfer_searcher import BoTorchTransfer

# Partly based on https://github.com/awslabs/syne-tune/blob/main/examples/launch_simulated_benchmark.py

def get_configs(backend: str, xgboost_res_file: str = None, simopt_backend_file: str = None,
                yahpo_dataset: str = None, yahpo_scenario: str = None):
    """
    Produce get_backend function required by do_tasks_in_order
    """
    if backend == 'SimOpt':
        assert simopt_backend_file is not None, 'SimOpt backend requires a problem file.'
        task_list = list(range(0, 9))
        return task_list, lambda active_task_val: simopt_backend_conf(
            simopt_backend_file, active_task_val
        )
    if backend == 'XGBoost':
        assert xgboost_res_file is not None, 'XGBoost requires a file with experiment evaluations.'
    ...
    if backend == 'YAHPO':
        ...
        print('Using yahpo scenario %s and data set %s.' %(yahpo_scenario, yahpo_dataset))
...
def massage_data(res):
    ...
    try: 
        ...
        data_sizes = np.array([int(min(0.8*70000,70000*10**ii)) 
                           for ii in np.linspace(-3, 0, 28)])
...
def xgboost_blackbox(X, Y, order, data_sizes, E, data_size):
    ...
    hyperparameters = pd.DataFrame(
        data=X.T, columns=order+['data_size']
    )
    config_space = {
...
def get_points_to_evaluate_myoptic(df, past_sizes, n_points, conf_space, past_points,
                                   active_task_str, metric):
    return []
...
def get_transfer_points_active(raw_df, past_sizes, n_points, conf_space, tasks,
                               active_task_str, metric):
...
def simopt_backend_conf(backend, active_task_val):
    ...
    trial_backend=ActiveLocalBackend(entry_point=backend,
                                     active_task_val=active_task_val,
                                     active_task_str='time_idx')
    
    callbacks = []
    return trial_backend, conf_space, callbacks

def xgboost_backend_conf(X, Y, order, active_task_list, E, active_task_val):
    blackbox = xgboost_blackbox(X, Y, order, data_sizes=active_task_list, E=E, 
                                data_size=active_task_val)
...
def _get_task_set(keys_to_use, hyp_df, metric_vals, opt_mode):
    return set([ _make_config_str(keys_to_use, opt_idx, hyp_df)
        for opt_idx in _get_optima_idx(opt_mode, metric_vals) ])

def num_optima(past_points, active_task_str, conf_space, metric, opt_mode, n_points, past_df=None):
    """
    ...
    if past_df is not None:
        ...
        filt_past_df = get_first_n_df(past_df, n_points, list(conf_space.keys())+[metric])
    ...
    for task_id in past_points:
        ...
        list_best_configs.append(
            _get_task_set(keys_to_use, past_points[task_id].hyperparameters, vals, opt_mode)
        )
...
def initialise_scheduler_stopping_criterion(
    optimiser, base_kwargs, transfer_kwargs, points_per_task, past_points,
    check_enough_tasks, active_task_val
):
...
def do_tasks_in_order(seed, active_task_list, pte_func, points_per_task,
                      get_backend, optimiser='BayesianOptimization',
                      metric='metric_error', opt_mode='min',
                      active_task_str=None, uses_fidelity=False,
                      n_workers=4):
    assert active_task_str is not None

    _, conf_space, _ = get_backend(
                    active_task_val=active_task_list[0])

    num_optima_wr = lambda past_points, past_df=None : num_optima(
        past_points=past_points, active_task_str=active_task_str,
        conf_space=conf_space, metric=metric, opt_mode=opt_mode,
        n_points=points_per_task, past_df=past_df)

    warm_up_its = {'ZeroShot': 1,
                   'WarmBO': 1,
                   'PrevBO': 1,
                   'PrevNoBO': 1,
                   'WarmBOShuffled': 1,
    ...
    for active_task_val in active_task_list:
        ...
        past_points = pte_func(past_df, past_task_vals, points_per_task, 
                               conf_space, past_points, active_task_str, metric)
        ...
        check_enough_tasks = \
            num_optima_wr(past_points) >= warm_up_its[optimiser] if optimiser in warm_up_its else True
