...
def evaluate_problem_price(prob: Problem, xx, rand_gen, reps: int = 1):
    ...
    profit = np.mean([prob.model.replicate([rand_gen])[0]['profit']
                      for _ in range(reps)])
...
def evaluate_problem(prob: Problem, xx, rand_gen, reps: int = 1):
    ...
    # Simulate the feature vector performance
    return np.mean([prob.model.replicate([rand_gen])[0]['profit']
                    for _ in range(reps)])

def plot_problem(ax, ii: int, results: List[float], task_num: int, N: int,
                 num_cols:int, vmin: float, vmax: float):
    """
    ...
    for jj in range(len(y_axs)):
        ax.scatter(x_axs[jj], y_axs[jj], marker='X', c='k',
                  s=100, edgecolors='white', linewidths=1.75)
