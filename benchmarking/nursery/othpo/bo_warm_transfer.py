...
def return_optima_from_task(task: TransferLearningTaskEvaluations, opt_mode: str, 
                            keys_to_use: list, metric: str, nn: int, de_str_dict):
    optima_idx = get_optima_but_nn_idx(opt_mode, task.objective_values(metric)[:,0,0], nn)
    if len(optima_idx) == 0:
        return None, None
    opt_val = task.objective_values(metric)[optima_idx[0],0,0]
    return opt_val, set([ _make_config_str(keys_to_use, opt_idx, task.hyperparameters, de_str_dict)
        for opt_idx in optima_idx ])

def get_optima_all_tasks(transfer_learning_evaluations, opt_mode, keys_to_use, metric, nn, de_str_dict):
    all_optima = []
    for task_id in transfer_learning_evaluations:
        opt_val, opt_configs = return_optima_from_task(transfer_learning_evaluations[task_id],
                                                      opt_mode, keys_to_use, metric, nn, de_str_dict)
        if opt_val is not None:
            all_optima.append((task_id, opt_val, opt_configs))
    return all_optima

def get_sorted_optima(transfer_learning_evaluations, opt_mode, keys_to_use, metric, nn, de_str_dict,
                      sort_by_task_id, shuffle_order):

    optimal_points = copy.deepcopy(get_optima_all_tasks(
        transfer_learning_evaluations, opt_mode, keys_to_use, metric, nn=nn, de_str_dict=de_str_dict))
...
def choose_warm_starters(num_warm, transfer_learning_evaluations, opt_mode, keys_to_use, metric,
                        sort_by_task_id, shuffle_order):
    points_to_evaluate = []
    nn = 0
    ...
    optimal_points = get_sorted_optima(transfer_learning_evaluations, opt_mode, keys_to_use,
                                       metric, nn, de_str_dict, sort_by_task_id, shuffle_order)
...
class WarmStartBayesianOptimization(BoTorch):
    ...
    exploration.
    """
    def __init__(self, config_space: Dict[str, Any], metric: str, 
                 transfer_learning_evaluations: Dict[Any, TransferLearningTaskEvaluations], 
                 num_warm_points: int, points_to_evaluate=[], sort_by_task_id=None,
                 shuffle_order=False, **kwargs):
        
        assert len(transfer_learning_evaluations) >= 1, 'need a task to transfer from'
        
        if sort_by_task_id:
            assert np.array([type(key) in [int, float] for key in transfer_learning_evaluations.keys()]).all(), "task ids should be numeric if we're sorting by them."
        ...
        warm_points = choose_warm_starters(
            num_warm_points, transfer_learning_evaluations, kwargs['mode'], 
            config_space.keys(), metric, sort_by_task_id, shuffle_order)
