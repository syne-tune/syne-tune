...
def get_data_splits(seed, X, Y, test_threshold,
                    smallest_train):
    np.random.seed(seed)
...
def get_log_lik(model, X, Y):
    return np.sum(
        [model.predict_log_proba(X)[kk,Y[kk]] 
        for kk in range(len(Y))])

def get_error_count(model, X, Y):
    return np.sum(model.predict(X) != Y)

def train_and_evaluate(X_train, Y_train, par_dict, X_test, Y_test,
                       model_name, llik=True, seed=0):
    if model_name == 'XGBClassifier':
        max_depth = par_dict['max_depth'] \
            if 'max_depth' in par_dict else None
        learning_rate = par_dict['learning_rate'] \
            if 'learning_rate' in par_dict else None
        reg_alpha = par_dict['reg_alpha'] \
            if 'reg_alpha' in par_dict else None
        min_child_weight = par_dict['min_child_weight'] \
            if 'min_child_weight' in par_dict else None
        reg_lambda = par_dict['reg_lambda'] \
            if 'reg_lambda' in par_dict else None
        n_estimators = par_dict['n_estimators'] \
            if 'n_estimators' in par_dict else None
