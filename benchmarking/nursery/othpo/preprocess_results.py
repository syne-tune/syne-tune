...
def preprocess_res(**kwargs):
    ...
    for res_ff in res_files:
        ...
        for key in res_keys:
            ...
            for task_val in task_values:
                ...
                if opt_mode == 'max':
                    line = [np.max(scores.iloc[:ii+1]) for ii in range(points_per_task)]
                else:
                    line = [np.min(scores.iloc[:ii+1]) for ii in range(points_per_task)]
    ...
    for method in methods:
        for task_val in task_values:
            ...
            std_mean_performance[method][task_val] = np.std(lines, 0, ddof=1)/np.sqrt(len(lines))
    ...
    for method in methods:
        for task_val in task_values:
            max_means[task_val] = max(max_means[task_val], np.max(mean_performance[method][task_val]))
            min_means[task_val] = min(min_means[task_val], np.min(mean_performance[method][task_val]))
    if opt_mode == 'max':
    ...
    for method in methods:
        ...
        for task_val in task_values:
            proj = (worst_means[task_val] - np.array(evaluation_lines[method][task_val]) ) / (
    ...
    for method in methods:
        for task_val in task_values:
            normalised_scores = 100 * (evaluation_lines[method][task_val] - best_means[task_val]) / (
                mean_performance['RandomSearch'][task_val][points_per_task-1] - best_means[task_val])

            mean_normalised_score[method][task_val] = np.mean(normalised_scores, 0)
            std_mean_normalised_score[method][task_val] = np.std(normalised_scores, 0, ddof=1)/np.sqrt(len(normalised_scores))
    ...
    preprocessed_results['preprocessing_timestamp'] = datetime.datetime.now().strftime('%Y-%m-%d-%H-%M-%S')

    os.makedirs('plotting', exist_ok=True)
    pickle.dump(preprocessed_results, open('plotting/preprocessed_results_%s.p' %kwargs["experiment"], 'wb'))
    print("Preprocessing completed")


def get_parser():
    parser = argparse.ArgumentParser()
    parser.add_argument("experiment", type=str, help="What experiment to preprocess the results of" )
    return parser
...
if __name__ == "__main__":
    ...
    preprocess_res(**experiments_meta_dict[args.experiment], experiment=args.experiment,
                   points_per_task=points_per_task, qq_values=qq_values)
