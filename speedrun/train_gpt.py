...
def distributed_data_generator(
    filename_pattern: str, seq_len: int, grad_accum_steps: int, align_to_bos: bool
):
    rank = dist.get_rank()
    world_size = dist.get_world_size()
    ...
    while True:
        ...
        )  # provide buffer to handle samples up to length seq_len
        if pos + token_window + 1 >= len(tokens):
            tokens = _load_data_shard(next(file_iter))
            pos = 0
        for _ in range(grad_accum_steps):
            if align_to_bos:
                batch_starts, tokens_consumed = find_batch_starts(
                    tokens, pos, seq_len, token_window
                )
...
class Hyperparameters:
    ...
    cooldown_frac: float = (
        0.45  # fraction of training spent cooling down the learning rate
    )
...
optimizer2 = Muon(
    hidden_matrix_params, lr=args.muon_lr, momentum=args.muon_momentum, weight_decay=0.0
)
...
train_loader = distributed_data_generator(
    args.train_files, args.train_seq_len, grad_accum_steps, align_to_bos=True
)
...
for step in range(train_steps + 1):
    ...
    if last_step or (args.val_loss_every > 0 and step % args.val_loss_every == 0):
        ...
        val_loader = distributed_data_generator(
            args.val_files, args.val_seq_len, grad_accum_steps, align_to_bos=False
        )
        val_loss = 0
        with torch.no_grad():
        ...
        if master_process:
            ...
            reporter(
                ...
                iteration=iteration,
                train_time=training_time_ms / max(step, 1),
            )
