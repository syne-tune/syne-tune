...
def load_nb201(dataset_name, nb201_random_seed):
    ...
    df_val_acc = pd.DataFrame((1.0-bb_dict[dataset_name].objectives_evaluations[:, nb201_random_seed, :, metric_valid_error_dim]) * 100,
    ...
    df_time = pd.DataFrame(bb_dict[dataset_name].objectives_evaluations[:, nb201_random_seed, :, metric_runtime_dim][:, -1],
...
def analyse_experiment(experiment_name, nb201_df, reference_time=None):
    ...
    condition = (nb201_df[table_hp_names] ==
