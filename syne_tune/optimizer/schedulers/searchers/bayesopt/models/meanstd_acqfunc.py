...
from syne_tune.optimizer.schedulers.searchers.bayesopt.tuning_algorithms.base_classes \
    import SurrogateModel, AcquisitionFunction, SurrogateOutputModel, \
...
class MeanStdAcquisitionFunction(AcquisitionFunction, ABC):
    ...
    def __init__(
        ...
        self.model_output_names = \
            [self.active_metric] + output_names[:active_pos] + \
    ...
    def _get_num_fantasies(
            self, output_to_predictions: PredictionsPerOutput) -> int:
        """
        ...
        assert len(num_fantasy_values) <= max_size, \
            "Predictive means have inconsistent numbers of fantasies: " +\
            str(num_fantasy_values)
        return max(list(num_fantasy_values))

    def _get_current_bests(
    ...
    def _get_current_bests_internal(
        ...
        """
        active_metric_current_best = model[
            self.active_metric].current_best()
        return ActiveMetricCurrentBestProvider(
            active_metric_current_best)
    ...
    def _add_head_gradients(
            grad1: Dict[str, np.ndarray],
            grad2: Optional[Dict[str, np.ndarray]]) -> Dict[str, np.ndarray]:
        if grad2 is None:
            return grad1
        else:
    ...
    def _map_outputs_to_predictions(
            ...
            inputs: np.ndarray) -> PredictionsPerOutput:
        return {
    ...
    def _extract_mean_and_std(
            ...
            metric: str = None) -> (np.ndarray, np.ndarray):
        if metric is None:
