    ...
    def sample_marginals(
        ...
        def sample_func(resource: int, n_samples: int):
            return self._states[resource].sample_marginals(
                test_features, num_samples=n_samples, random_state=random_state
            )
...
class HyperTuneJointGPPosteriorState(GaussProcPosteriorState):
    ...
    def _extend_features_by_resource(
        ...
        hp_range = HyperparameterRangeInteger(
            name="resource",
            lower_bound=self._resource_attr_range[0],
            upper_bound=self._resource_attr_range[1],
            scaling=LinearScaling(),
        )
        resource_enc = hp_range.to_ndarray(resource).item()
        shape = (getval(test_features.shape[0]), 1)
        extra_col = anp.full(shape, resource_enc)
