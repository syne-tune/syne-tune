...
class ExtendFeaturesByResourceMixin:
    ...
    def _extend_features_by_resource(self, test_features: np.ndarray) -> np.ndarray:
...
class PosteriorStateClampedResource(
    ...
    """
    Converts posterior state of :class:`GaussProcPosteriorState` over extended
    inputs into posterior state over non-extended inputs, where the resource
    attribute is clamped to a fixed value.
    """
    ...
    def predict(self, test_features: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        return self._poster_state_extended.predict(
            self._extend_features_by_resource(test_features)
        )
    ...
    def sample_marginals(
        ...
        return self._poster_state_extended.sample_marginals(
            test_features=self._extend_features_by_resource(test_features),
...
class MeanFunctionClampedResource(MeanFunction, ExtendFeaturesByResourceMixin):
    ...
    def forward(self, X):
        return self._mean_extended.forward(self._extend_features_by_resource(X))
...
class KernelFunctionClampedResource(KernelFunction, ExtendFeaturesByResourceMixin):
    ...
    def diagonal(self, X):
        return self._kernel_extended.diagonal(self._extend_features_by_resource(X))

    def diagonal_depends_on_X(self):
        return self._kernel_extended.diagonal_depends_on_X()

    def forward(self, X1, X2):
        X1_ext = self._extend_features_by_resource(X1)
        if X2 is X1:
            X2_ext = X1_ext
        else:
            X2_ext = self._extend_features_by_resource(X2)
...
def hypertune_ranking_losses(
    ...
    if independent_models:
        ...
        mean_max_resource = MeanFunctionClampedResource(
            mean_extended=poster_state.poster_state.mean,
            resource=max_resource,
            resource_attr_range=resource_attr_range,
        )
        kernel_max_resource = KernelFunctionClampedResource(
            kernel_extended=poster_state.poster_state.kernel,
            resource=max_resource,
            resource_attr_range=resource_attr_range,
        )
        noise_variance = poster_state.poster_state.noise_variance
