...
class SliceSampler(object):
    def __init__(
            self, log_density: Callable[[np.ndarray], float], scale: float,
    ...
    def _gen_next_sample(self, x0: np.ndarray) -> np.ndarray:
        ...
        lower_bound, upper_bound = slice_sampler_step_out(log_pivot, self.scale, sliced_log_density, self._random_state)
        movement = slice_sampler_step_in(lower_bound, upper_bound, log_pivot, sliced_log_density, self._random_state)
...
def gen_random_direction(
        dimension: int, random_state: RandomState) -> np.ndarray:
...
def slice_sampler_step_out(
        log_pivot: float, scale: float,
        sliced_log_density: Callable[[float], float],
        random_state: RandomState) -> Tuple[float, float]:
    r = random_state.rand()
    lower_bound = -r * scale
...
def slice_sampler_step_in(
        lower_bound: float, upper_bound: float, log_pivot: float,
        sliced_log_density: Callable[[float], float],
        random_state: RandomState) -> float:
