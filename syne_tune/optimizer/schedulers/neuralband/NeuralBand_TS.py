...
class NeuralbandTSScheduler(FIFOScheduler):
    ...
    def __init__(self, config_space, gamma = 0.01, nu = 0.01, step_size = 30, max_while_loop = 100,  **kwargs):
        ...
        if self.mode == "min":
            ...
            self.train_step_size = step_size*2
        ...
        if len(result) == 0:
            ...
            if trial_id not in self._active_trials:
                ...
                if self.mode == "min":
                    ...
                    if test_loss > self.currnet_best_score:
                        self.currnet_best_score = test_loss
                    perturbed_loss = (test_loss + np.random.normal(0, 0*self.nu* 1* (100.0- test_loss) *(1 - hp_budget)))/100.0
