...
class NeuralbandUCBScheduler(FIFOScheduler):
            ...
            if config is not None:
                ...
                if self.mode == "min":
                    ...
                    if while_loop_count > self.max_while_loop:
                        ...
                        #print("force to select:",l_t_score[0][1])
                        break
        ...
        if len(result) == 0:
            ...
            if trial_id not in self._active_trials:
                ...
                if self.mode == "min":
                    ...
                    perturbed_loss = test_loss + np.random.normal(0, 0*self.nu* self.currnet_best_score *(1 - hp_budget))
                else:
                    if test_loss > self.currnet_best_score:
                        self.currnet_best_score = test_loss
                    perturbed_loss = (test_loss + np.random.normal(0, self.nu* 0* (100.0- test_loss) *(1 - hp_budget)))/100.0
