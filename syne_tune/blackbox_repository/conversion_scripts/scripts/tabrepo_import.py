...
CONFIGURATION_SPACE_CatBoost = {
    ...
    'l2_leaf_reg': uniform(lower=1, upper=5,),
...
def generate_tabrepo(config_space):
    ...
    metrics = repo.metrics(datasets=repo.datasets(),
...
def convert_dataset(config_space):
    ...
    objective_names = ["metric_error", "metric_error_val", "metric_time_train_s", "metric_time_infer_s", "metric_rank"]
...
class TABREPORandomForest(TABREPORecipe):
    def __init__(self):
        super().__init__(CONFIGURATION_SPACE_RandomForest, BLACKBOX_NAME + 'RandomForest')
