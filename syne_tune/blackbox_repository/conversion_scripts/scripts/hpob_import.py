...
SEARCH_SPACE_7609 = {
    ...
    "positions_categorical": {"respect": [6, 7], "replace": [8, 9]},
    "config_space": {
        "num.trees": uniform(0.0, 1.0),
        "mtry": uniform(0.0, 1.0),
        "min.node.size": uniform(0.0, 1.0),
...
def convert_dataset(search_space, dataset):
    ...
    for i, config in enumerate(dataset["X"]):
        ...
        for hp in search_space["positions_hps"]:
            pos = search_space["positions_hps"][hp]
            final_config.append(config[pos])

        # Handle categorical hyperparameters
        if search_space["positions_categorical"]:
            for hp in search_space["positions_categorical"]:
                positions = search_space["positions_categorical"][hp]
                if hp == "booster":
                    if config[positions[0]] == 1.0:
                        final_config.append(0.0)
                    else:
                        final_config.append(1.0)
                elif hp == "respect":
                    if config[positions[0]] == 1.0:
                        final_config.append(0.0)
                    else:
                    ...
                    if config[positions[0]] == 1.0:
                        final_config.append(0.0)
                    elif config[positions[1]] == 1.0:
                        final_config.append(1.0)
                    else:
                        final_config.append(2.0)
