...
from sagemaker_tune.backend.backend import BACKEND_TYPES, get_backend_type

# this is required so that metrics are written
from sagemaker_tune.backend.sagemaker_backend.instance_info import InstanceInfo, InstanceInfos
...
class Reporter:
    ...
    # Whether to add automatically `smt_worker_time` information on the metric reported by the worker which measures
    # the number of seconds spent since the creation of the Reporter.
    add_time: bool = True

    # Whether to add automatically `smt_worker_cost` information on the metric reported by the worker which measures
    ...
    def __call__(self, **kw) -> None:
        ...
        A time stamp `smt_timestamp` is added, if `add_time` is True then `smt_worker_time` is added
        which measures seconds spent in the worker, if `add_cost` is True `smt_worker_cost` is added that measures
        dollar cost spent in worker (only available on Sagemaker instances and requires `smt_worker_time` to be
        activated).
        :param kwargs: key word arguments of the metrics to report, for instance `report(epoch=1, loss=1.2)` the only
        constrain is that values must be serializable with json and keys should not start with `smt_` which is a
        reserved namespace for SagemakerTune internals.
        """
        for key in kw.keys():
            assert not key.startswith("smt_"),\
                "The metric prefix 'smt_' is used by Sagemaker Tune internals, " \
                "please use a metric name that does not start with 'smt_'."
