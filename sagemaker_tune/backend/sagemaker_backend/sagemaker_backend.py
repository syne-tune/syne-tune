...
from sagemaker_tune.constants import SMT_INSTANCE_TYPE, SMT_INSTANCE_COUNT, SMT_CHECKPOINT_DIR
...
from sagemaker_tune.backend.sagemaker_backend.sagemaker_utils import \
    ...
    add_sagemaker_tune_dependency, map_identifier_limited_length
...
class SagemakerBackend(Backend):
    ...
    def __init__(
        ...
        if self.sm_estimator.base_job_name is None:
            base_job_name = smt_prefix
        else:
            base_job_name = smt_prefix + self.sm_estimator.base_job_name
        # Make sure len(base_job_name) <= 63
        self.sm_estimator.base_job_name = map_identifier_limited_length(
            base_job_name)

        add_sagemaker_tune_dependency(self.sm_estimator)
    ...
    def _schedule(self, trial_id: int, config: Dict):
        ...
        # with reserved names `smt_instance_type` and `smt_instance_count`.
        ...
        # `smt_instance_type` or `smt_instance_count` in the config space.
