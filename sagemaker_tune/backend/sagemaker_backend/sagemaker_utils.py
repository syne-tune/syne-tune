...
from sagemaker_tune.backend.trial_status import TrialResult
...
def add_sagemaker_tune_dependency(sm_estimator):
    # adds code of sagemaker tune to the estimator to be sent with the estimator dependencies so that report.py or
    # other functions of sagemaker tune can be found
    sm_estimator.dependencies = sm_estimator.dependencies + [str(Path(sagemaker_tune.__path__[0]))]
