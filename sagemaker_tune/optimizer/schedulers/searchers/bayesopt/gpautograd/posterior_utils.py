...
from sagemaker_tune.optimizer.schedulers.searchers.bayesopt.gpautograd.constants\
    import NOISE_VARIANCE_LOWER_BOUND, MIN_POSTERIOR_VARIANCE, \
    MIN_CHOLESKY_DIAGONAL_VALUE
from sagemaker_tune.optimizer.schedulers.searchers.bayesopt.gpautograd.custom_op \
    import AddJitterOp, cholesky_factorization, flatten_and_concat
...
def sample_posterior_marginals(
    ...
    n01_vecs = [random_state.normal(size=getval(post_means.shape))
...
def sample_posterior_joint(
    ...
    n01_vecs = [random_state.normal(size=getval(posterior_mean.shape))
                for _ in range(num_samples)]
...
def sample_and_cholesky_update(
        features, chol_fact, pred_mat, mean, kernel, noise_variance,
        feature, random_state: RandomState, mean_impute_mask=None):
