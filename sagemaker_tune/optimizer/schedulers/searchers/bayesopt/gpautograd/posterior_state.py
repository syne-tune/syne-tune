...
from sagemaker_tune.optimizer.schedulers.searchers.bayesopt.gpautograd.mean \
    import MeanFunction
from sagemaker_tune.optimizer.schedulers.searchers.bayesopt.gpautograd.posterior_utils \
    import cholesky_computations, predict_posterior_marginals, \
    ...
    def backward_gradient(
        ...
        def diff_test_feature(test_feature_array):
            ...
            if has_std:
                ...
                pred_std_sum = anp.sum(
                    anp.multiply(pred_std, head_gradients_std))
            else:
                pred_std_sum = 0.0
...
class IncrementalUpdateGPPosteriorState(GaussProcPosteriorState):
    ...
    def sample_and_update(
            self, feature: np.ndarray, mean_impute_mask=None,
        ...
        chol_fact_new, pred_mat_new, features_new, target = \
            sample_and_cholesky_update(
                self.features, self.chol_fact, self.pred_mat, self.mean,
                self.kernel, self.noise_variance, feature, random_state,
                mean_impute_mask)
        state_new = IncrementalUpdateGPPosteriorState(
            features=features_new,
            targets=None,
